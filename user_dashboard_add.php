...
if( is_user_logged_in() ) {
	...
	if( $user_role !=4 ){
		echo '<h2 style="text-align: center; margin-top:120px;">Only Sellers can add property</h2>';
		echo '<style>footer{display:block !important;}</style>';
		get_footer(); exit;
	}
}
else {
	echo '<h2 style="text-align: center; margin-top:120px;">Please login or register as a Seller using the "Account" button on top right.</h2>';
	echo '<style>footer{display:block !important;}</style>';
	get_footer(); exit;
}
       ...
       if($has_errors){
        ...
        if( $edited==1) {
            ...
            foreach($feature_list_array as $key => $value){
                ...
                $post_var_name    =   wpestate_limit45(sanitize_title( $post_var_name ));
                $post_var_name    =   sanitize_key($post_var_name);
                if($post_var_name!='' && isset($_POST[$post_var_name])){
                    $feature_value=wp_kses( esc_html($_POST[$post_var_name]),$allowed_html );
                    update_post_meta($post_id, $post_var_name, $feature_value);
                }
            }
